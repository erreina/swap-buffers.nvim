-- A simple NeoVim plugin for swapping windows
local api = vim.api
local fn = vim.fn
local M = {}
...
local function get_windows_keys(windows)
  local mapping = {}
...
function M.pick()
  ...
  local target_window_id = window_keys[key]
  local extra = {}
  local choices = 0
  ...
  if choices > 1 then
    ...
    if second then
      ...
      target_window_id = window_keys[combined] or window_keys[key]
    else
  ...
  if target_window_id then
    ...
    local target_buffer_nr = vim.fn.winbufnr(target_window_id)
